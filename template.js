...
if (isLoggingEnabled) {
    logToConsole(JSON.stringify({
        ...
        'Type': 'Request',
        'TraceId': traceId,
        'EventName': mappedEventData.description ? mappedEventData.description : mappedEventData.eventId,
        'RequestMethod': 'POST',
        'RequestUrl': postUrl,
...
}, {headers: {'Content-Type': 'application/json', 'Authorization': 'Bearer '+data.accessToken}, method: 'POST'}, JSON.stringify(postBody));
    ...
    if (mappedData.identifiers) {
        for (let key in mappedData.identifiers) {
            if (mappedData.identifiers[key]['hashed_email']) {
                mappedData.identifiers[key]['hashed_email'] = hashData(mappedData.identifiers[key]['hashed_email']);
            }
        }
    }
    ...
    if (mappedData.identifiers) {
        ...
        for (let userDataKey in mappedData.identifiers) {
            if (mappedData.identifiers[userDataKey]['hashed_email']) {
                userData.push({'hashed_email': mappedData.identifiers[userDataKey]['hashed_email']});
            }

            if (mappedData.identifiers[userDataKey]['twclid']) {
                userData.push({'twclid': mappedData.identifiers[userDataKey]['twclid']});
            }
        }
